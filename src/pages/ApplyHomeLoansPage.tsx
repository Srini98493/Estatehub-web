...
export const ApplyHomeLoansPage = () => {
  ...
  const { data: services, isLoading } = useQuery({
    ...
    queryFn: async () => {
      try {
        const response = await api.get(API_ENDPOINTS.GET_HOME_LOAN_SERVICES);
        if (!response?.data) {
          return [];
        }
    ...
    return (
      <PageLayout title="Home Loans">
        <div className="text-center py-12">
          <p className="text-gray-600 text-lg">Please sign in to view and apply for home loans</p>
          {/* <Button
  ...
  return (
    <PageLayout title="Home Loans" headerContent={headerContent}>
